...
Phase A は **core 完了**。Phase B は **BFF canonical API、customer intelligence endpoint、frontend adapter 接続まで実装済み** と判定する。
...
- tests / verification
  - contracts、sources、intelligence、cli、bff、frontend の typecheck / test を `bun run verify` で実行する構成になっている。

## Phase B: demo BFF

### 判定

Phase B の初回スコープである **frontend demo 用 read-only product API boundary** は、BFF 側で成立している。

frontend はこのリポジトリの `apps/frontend` に統合済み。Customers、Wallet 360°、Co-usage Patterns の 3 画面は BFF canonical response を API client / adapter で UI DTO に変換して表示する構成になっている。Setup はローカル UI 状態を作る画面で、BFF product endpoint には依存しない。

ただし、現状の自動検証は typecheck / unit test / BFF route test が中心で、ブラウザ上での通し操作や E2E は未整備。
...
- `apps/bff`
  ...
  - `GET /customers/:address/intelligence` は保存済み read model を返し、request path で live source を呼ばない。
...
- `apps/frontend`
  ...
  - API response は `packages/contracts` の Phase B validator で検証してから adapter に渡す。
  - `lib/api/adapters.ts` が canonical response を既存 UI DTO に変換する。
  - Customers、Wallet 360°、Co-usage Patterns の 3 画面が BFF-backed data を表示する。
  - `providerId` は現時点では routing / 表示用で、BFF query や provider scope filter には使っていない。
...
- frontend integration の検証
  - API client / adapter / 3画面の接続は `apps/frontend` に実装済み。
  - adapter の canonical response -> UI DTO 変換は単体テストで確認している。
  - 次は実際の Next.js app と BFF を起動したブラウザ操作、または E2E による demo flow 検証を追加する。
...
次にやるべきことは、**Phase B の demo flow 検証と scope 整理**。

優先順は次の通り。

1. BFF と frontend を同時起動し、Setup、My Customers、Wallet 360°、Co-usage Patterns の demo flow をブラウザで確認する。
2. 必要なら E2E または smoke test を追加し、BFF-backed 3画面の最低限の通し検証を自動化する。
3. provider scope / endpoint scope を query/header/API contract に入れるか、demo routing 表示用途に留めるかを決める。
4. 必要になった場合のみ、`GET /patterns` の独立 endpoint を contract-first で追加する。

## One-line summary

Phase A の data foundation は通過済み。Phase B は BFF canonical API、customer intelligence endpoint、frontend adapter 接続まで完了しており、次は demo flow の通し検証と scope 整理を進める。
