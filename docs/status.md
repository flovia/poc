...
Phase A は **core 完了**。Phase B は **demo BFF 初回実装完了** と判定する。

```text
Phase A core: done
Phase A hardening: follow-up possible
Phase B demo BFF: initial done
...
- deterministic demo read model
  - `apps/bff/src/data/phase-b-demo.ts` の deterministic fixture / read model を返す。
  - module initialization 時に `packages/contracts` の validator で検証する。
- docs
...
- BFF demo data の外部 fixture 化 / projection store 化
  - 現状は `apps/bff/src/data/phase-b-demo.ts` の in-file deterministic read model。
  - 次は JSON fixture、generated read model、または projection builder に分離し、再生成・差し替え可能にする。
...
3. BFF demo read model を in-file 定数から fixture / projection builder に分離する。
4. frontend 4画面（Setup、My Customers、Wallet 360°、Co-usage Patterns）で demo flow を通す。
5. 必要になった場合のみ、`GET /patterns` の独立 endpoint を contract-first で追加する。

## One-line summary

Phase A の data foundation は通過済み。Phase B は BFF 側の read-only demo API 初回実装まで完了しており、次は frontend adapter 接続と demo read model の fixture / projection 化を進める。
