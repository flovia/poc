...
- Customers / Customer Profile 画面向けに、ウォレット単位の観測を customer projection として提供する
...
| `GET /customers` | Customers 画面向けの customer projection 一覧。payer wallet を customer 候補として扱う |
| `GET /customers/:address/profile` | Customer Profile 画面向けの詳細 projection。metrics、provider usage、timeline、insights を返す |
...
curl http://localhost:3001/customers/0xpayer/profile
```
...
- `customers` 系 endpoint の customer は、payer wallet を画面向けに投影した PoC 用語です。ウォレットアドレスを verified human identity として断定しません。
...
`GET /customers` は一覧画面向けに、次のような軽量 row を返します。
...
```json
{
  ...
  "spendAtomic": "3000000",
  "providerCount": 2,
  "lastSeenAt": 1777333060,
  "activityGrowth": 0,
  "upsellOpportunity": "medium"
}
```

`GET /customers/:address/profile` は詳細画面向けに、`customer`、`metrics`、`providers`、`timeline`、`insights` をまとめて返します。`activityGrowth`、`freeTierProgress`、`entryPointRatio`、`upsellOpportunity` は PoC 用の決定的な heuristic であり、production-grade な revenue analytics ではありません。
