...
import { getProviderLeaderboards } from "@/lib/data-source";

export default async function ProviderLeaderboardPage() {
  const [dataMode, leaderboards] = await Promise.all([
    getServerDashboardMode(),
    getProviderLeaderboards(100),
  ]);
  ...
  return (
    <>
      ...
      <AppShell activeProviderId={undefined} activeRoute="rankings" dataMode={dataMode}>
            ...
            metrics: [
              {
                ...
                description:
                  "The leaderboard ranks providers currently present in the snapshot, not the entire market.",
                icon: "external",
              },
              {
                label: "Payment txns",
        ...
        />
        <ProviderLeaderboardScreen
          transactions={leaderboards.transactions}
          settledAmount={leaderboards.settledAmount}
        />
      </AppShell>
    </>
  );
