...
const make = (overrides: Partial<StoredProvider> & { providerId: string; name: string }): StoredProvider => ({
  mode: "simple",
  payTo: "0xabc",
  createdAt: 0,
  ...overrides,
...
describe("filterProviders", () => {
  ...
  test("query matches serviceId", () => {
    const r = filterProviders(providers, { ...DEFAULT_PROVIDER_FILTER, query: "coingecko.com" }, ctx());
    expect(r.map((p) => p.providerId)).toEqual(["coingecko"]);
  });

  test("source 'pay-sh' excludes the coingecko generated row and demo/user rows", () => {
  ...
  test("chains filter is OR across selected chains", () => {
    const r = filterProviders(
      providers,
      { ...DEFAULT_PROVIDER_FILTER, chains: ["solana"] },
      ctx(),
    );
    expect(r.map((p) => p.providerId)).toEqual(["quicknode"]);
  });
  ...
  test("chains filter excludes providers with no chains", () => {
    const r = filterProviders(
      providers,
      { ...DEFAULT_PROVIDER_FILTER, chains: ["base"] },
      ctx(),
    );
    expect(r.map((p) => p.providerId)).toEqual(["quicknode", "coingecko", "user-1"]);
  });

  test("protocol 'x402' includes any provider whose protocols contain x402", () => {
    const r = filterProviders(
      providers,
      { ...DEFAULT_PROVIDER_FILTER, protocol: "x402" },
      ctx(),
    );
    expect(r.map((p) => p.providerId)).toEqual(["quicknode", "coingecko"]);
  });

  test("protocol 'MPP' excludes x402-only providers", () => {
    const r = filterProviders(
      providers,
      { ...DEFAULT_PROVIDER_FILTER, protocol: "MPP" },
      ctx(),
    );
    expect(r.map((p) => p.providerId)).toEqual(["quicknode"]);
  });
