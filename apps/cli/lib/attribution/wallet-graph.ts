...
export type WalletUsageGraph = {
  ...
  providerWallets: Array<{
    ...
    payerWallets: Array<{
      ...
      otherServiceCandidates: Array<{ caseId: string; candidateType: string; entityId: string | null; confidence: number; reasons: string[]; evidenceRefs: string[] }>;
    }>;
  }>;
};
...
export const buildWalletUsageGraph = (): WalletUsageGraph => {
  ...
  const providerWallets = [...claimGroups.entries()]
    ...
    .map(([payTo, payToClaims]) => {
      const matchingObservations = observations.filter((observation) => observation.recipient_wallet.toLowerCase() === payTo);
      const payerWallets = [...new Set(matchingObservations.map((observation) => observation.payer_wallet.toLowerCase()))]
        .sort()
        .map((wallet) => {
          const payerObservations = observations.filter((observation) => observation.payer_wallet.toLowerCase() === wallet);
          const payerObservationIds = new Set(payerObservations.map((observation) => observation.observation_id));
          return {
            wallet,
            observations: matchingObservations
              ...
              .map((observation) => ({
                ...
                evidenceRefs: [`observation:${observation.observation_id}`, `tx:${observation.tx_hash}`],
              })),
            otherServiceCandidates: candidates
              .filter((candidate) => payerObservationIds.has(candidate.observation_id))
              .filter((candidate) => ["provider_candidate", "service_candidate", "endpoint_candidate", "middleman_candidate", "market_candidate"].includes(candidate.candidate_type))
              .filter((candidate) => observationsById.get(candidate.observation_id)?.recipient_wallet.toLowerCase() !== payTo)
              .map((candidate) => {
              ...
              .sort((left, right) => left.caseId.localeCompare(right.caseId) || left.candidateType.localeCompare(right.candidateType)),
          };
        });
