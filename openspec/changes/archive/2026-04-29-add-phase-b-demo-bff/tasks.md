...
- [x] 3.3 Add tests for `GET /customers/:address/profile` returning `404` for unknown wallet.
...
- [x] 4.1 Implement `GET /customers` using prepared demo read model.
- [x] 4.2 Implement `GET /customers/:address/profile` with normalized address lookup and not-found handling.
- [x] 4.3 Implement `GET /wallet-usage-graph` using prepared nested graph read model.
...
- [x] 6.2 Defer `/patterns`, `/summary`, provider query/header scoping, and live SDK telemetry until explicitly required.
